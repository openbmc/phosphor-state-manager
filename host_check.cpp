...
bool checkFirmwareConditionRunning(sdbusplus::bus::bus& bus)
{
         ...
         boost::adaptors::reverse(mapperResponse))
    {
        for (const auto& serviceIter : services)
        {
            ...
            {
                error("Error reading HostFirmware condition, error: {ERR}, "
                      "service: {SERVICE} path: {PATH}",
                      "ERR", e, "SERVICE", service, "PATH", path);
                throw;
            }
        }
    }
...
bool isChassiPowerOn(sdbusplus::bus::bus& bus)
{
    ...
    {
        error("Error reading Chassis Power State, error: {ERR}, "
              "service: {SERVICE} path: {PATH}",
              "ERR", e, "SERVICE", CHASSIS_STATE_SVC, "PATH",
              CHASSIS_STATE_PATH);
        throw;
    }
    return false;
