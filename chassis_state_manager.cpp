    ...
    for (const auto& [path, services] : mapperResponse)
    {
        for (const auto& serviceIter : services)
        {
            ...
            try
            {
                ...
                if (status == decoratorServer::PowerSystemInputs::Status::Good)
                {
                    server::Chassis::currentPowerStatus(PowerStatus::Good);
                }
                else if (status ==
                         decoratorServer::PowerSystemInputs::Status::Fault)
                {
...
void Chassis::uPowerChangeEvent(sdbusplus::message::message& msg)
{
    ...
    // then call determineStatusOfUPSPower() to see if a power status change
    // is needed
