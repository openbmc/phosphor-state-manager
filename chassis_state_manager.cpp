...
void Chassis::determineStatusOfPower()
{
    ...
    // Find all implementations of the UPS interface
    ...
    {
        info("Mapper response for UPS interfaces is empty!");
        return;
    }

    // Only support 1 UPS per system
    if (mapperResponse.size() != 1)
    {
        error("More then one UPS interface detected!: {SIZE}", "SIZE",
              mapperResponse.size());
        throw;
    }
    ...
    for (const auto& [path, services] : mapperResponse)
    {
        for (const auto& serviceIter : services)
        {
            ...
            try
            {
                ...
                {
                    info("UPS is not present in system");
                    return;
                }
                ...
                if (std::get<uint>(batteryLevel) == 8)
                {
