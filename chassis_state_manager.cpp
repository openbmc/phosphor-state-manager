...
bool Chassis::stateNotActive(const std::string& target)
{
    ...
    {
        //True - then its powered down
        return true;
    }
    else
    {
        //False - then its powered up
        return false;
    }
                            ...
                            sd_bus_error* retError)
{
    ...
    if((newStateUnit == CHASSIS_STATE_POWEROFF_TGT) &&
       (newStateResult == "done") &&
       (stateNotActive(CHASSIS_STATE_POWERON_TGT) == true))
    {
        ...
        this->currentPowerState(server::Chassis::PowerState::Off);
            ...
            (newStateResult == "done") &&
            (stateNotActive(CHASSIS_STATE_POWERON_TGT) == false))
     {
