...
int main(int argc, char** argv)
{
    ...
    try
    {
            ...
            RestorePolicy::convertPolicyFromString(powerPolicy))
        {
            ...
            // the BMC reboot
            // auto size = std::snprintf(nullptr, 0, CHASSIS_LOST_POWER_FILE,
            // 0); size++; // null std::unique_ptr<char[]> buf(new char[size]);
            // std::snprintf(buf.get(), size, CHASSIS_LOST_POWER_FILE, 0);
            size_t hostId = 0;
            fs::path aclossPath{fmt::format(CHASSIS_LOST_POWER_FILE, hostId)};
            if (!fs::exists(aclossPath))
            {
                info(
