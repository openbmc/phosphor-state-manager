...
ChassisSMP::ChassisSMP(sdbusplus::bus_t& bus,
                       const sdbusplus::object_path& objPath, size_t id,
                       size_t numChassis) :
...
void ChassisSMP::startMonitoring()
{
    ...
    for (size_t i = 1; i <= numChassis; ++i)
    {
        ...
        sdbusplus::object_path chassisPath = fmt::format(CHASSIS_OBJ_PATH, i);
        ...
        debug("Chassis{CHASSIS_ID}: Monitoring chassis {MONITORED_CHASSIS_ID}",
              "CHASSIS_ID", id, "MONITORED_CHASSIS_ID", i);
    }
...
void ChassisSMP::aggregatePowerState()
{
    ...
    for (size_t i = 1; i <= numChassis; ++i)
    {
        ...
        sdbusplus::object_path chassisPath = fmt::format(CHASSIS_OBJ_PATH, i);
        std::string chassisService = fmt::format(CHASSIS_SERVICE, i);
        try
        {
                                        ...
                                        size_t chassisId)
{
    ...
    for (const auto& [property, value] : properties)
    {
        if (property == "CurrentPowerState")
        {
            ...
            else if (stateStr == "xyz.openbmc_project.State.Chassis.PowerState."
                                 "TransitioningToOff")
            {
                state = PowerState::TransitioningToOff;
            }
            else if (stateStr == "xyz.openbmc_project.State.Chassis.PowerState."
                                 "TransitioningToOn")
            {
                state = PowerState::TransitioningToOn;
            }
            ...
            {
                warning(
                    "Chassis{CHASSIS_ID}: Chassis {FAILED_CHASSIS_ID} is "
                    "transitioning to off while system is in {POWER_STATE}, "
                    "initiating power off for all chassis",
                    "CHASSIS_ID", id, "FAILED_CHASSIS_ID", chassisId,
