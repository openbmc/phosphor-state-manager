...
void ChassisSMP::startMonitoring()
{
    ...
    // Set up property change monitoring only for chassis present in inventory
    for (size_t i = 1; i <= numChassis; ++i)
    {
        if (!isChassisPresent(i))
        {
            info("Chassis{CHASSIS_ID}: Skipping monitoring for chassis "
