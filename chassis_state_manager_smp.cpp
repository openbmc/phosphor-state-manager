...
void ChassisSMP::aggregatePowerState()
{
    ...
    else if (hasOn)
    {
        ...
        // hold at TransitioningToOn. aggregatePowerState() is re-called
        // from sysStateChangeJobRemoved() once poweronTargetComplete is
        // set.
...
bool ChassisSMP::isChassisPresent(size_t chassisId)
{
    ...
    try
    {
        auto inventoryBusName =
            utils::getService(bus, inventoryPath, InventoryItem::interface);
...
void ChassisSMP::sysStateChangeJobRemoved(sdbusplus::message_t& msg)
{
    ...
    // poweronTargetComplete and re-run aggregation so that
    // aggregatePowerState() publishes PowerState::On once all leaf chassis
    // report On. We also remove the temporary chassis-on file here (on
    // JobRemoved) rather than during startup aggregation to avoid racing with
    // phosphor-reset-chassis-running@0.service, which writes the file
