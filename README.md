...
updating its corresponding D-Bus property. When using PSM, a user must ensure
all generic services installed within the PSM targets complete successfully in
order to have PSM properly report states.
...
ensures it's always getting the most accurate state information. It discovers
the chassis state by checking the `pgood` value from the power application. If
it determines that power is on then it will do the following:
...
- **Coordinated Power Control**: Power transition requests are forwarded to all
  ...
  power-offs to put them in the instance 0 obmc-chassis-power(off/on) targets.
