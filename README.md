...
- [drive][7]: The drive represents an individual storage device (NVMe/SSD) that
  supports power control. Drive instances use string identifiers (e.g. nvme0)
  rather than numeric IDs. The drive can be `Ready` (powered on), `Offline`
  (powered off), or `NotReady` (transitioning). `Unknown` indicates the state
  has not been determined and `UpdateInProgress` indicates a firmware update is
  active.
