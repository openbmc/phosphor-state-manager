...
bool Host::isAutoReboot()
{
    ...
    if(rebootCounter == 0)
    {
        log<level::INFO>("Host has already tried specified number of reboots");
        return false;
    }
...
int Host::readRebootCounter()
{
    ...
    if( file==NULL) {log<level::ERR>("Failed to open file rebootcounter\n"); return 0;}

    // Obtain the file size;
    r = fseek(file, 0, SEEK_END);
    if(r){
        log<level::ERR>("Fseek failed on rebootcounter\n");
        fclose(file);
        return 0;
    }
    ...
    if(r != lSize){
        log<level::ERR>("Failed to read rebootcounter\n");
        return 0;
    }
                         ...
                         sd_bus_error* retError)
{
       ...
       (newStateResult == "done"))
    {
            ...
            (newStateResult == "done"))
     {
         ...
         log<level::INFO>("Reset the reboot counter");
         Host::setRebootCounter(defaultRebootCounterValue);
     }
